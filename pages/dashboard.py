...
with left:
    ...
    cat_data = run_query("SELECT category, COUNT(*) AS count FROM tickets GROUP BY category ORDER BY count DESC")
    if cat_data:
...
if recent:
    ...
    def color_priority(val):
        colors = {"Critical": "background-color:#fee2e2;color:#991b1b",
    ...
    st.dataframe(df.style.map(color_priority, subset=["priority"]), use_container_width=True, hide_index=True)
else:
