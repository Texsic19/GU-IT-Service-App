...
rows = run_query("""
    SELECT t.*,
        ...
        u.phone AS submitter_phone, u.department,
...
PRIORITY_BADGE = {"Critical":("alert-triangle","#dc2626"),"High":("zap","#d97706"),"Medium":("info","#2563eb"),"Low":("check-circle","#16a34a")}
STATUS_BADGE   = {"Open":("circle-dot","#ca8a04"),"In Progress":("clock","#7c3aed"),"Resolved":("check-circle","#16a34a"),"Closed":("x-circle","#6b7280")}
CAT_ICON = {"Network":"wifi","Hardware":"cpu","Software":"monitor","Account/Access":"key","AV Equipment":"tv","Email":"mail","Printing":"printer"}

p_ico, p_col = PRIORITY_BADGE.get(t["priority"], ("circle-dot","#6b7280"))
s_ico, s_col = STATUS_BADGE.get(t["status"],     ("circle-dot","#6b7280"))
c_ico = CAT_ICON.get(t["category"], "wrench")
...
with b1:
    st.markdown(f'<div style="background:{s_col};border-radius:8px;padding:10px 14px;font-size:0.9rem;color:white;font-weight:500">{icon_text(s_ico, t["status"], 15, "white")}</div>', unsafe_allow_html=True)
with b2:
    st.markdown(f'<div style="background:{p_col};border-radius:8px;padding:10px 14px;font-size:0.9rem;color:white;font-weight:500">{icon_text(p_ico, t["priority"], 15, "white")}</div>', unsafe_allow_html=True)
with b3:
    st.markdown(f'<div style="background:#166534;border-radius:8px;padding:10px 14px;font-size:0.9rem;color:white;font-weight:500">{icon_text(c_ico, t["category"], 15, "white")}</div>', unsafe_allow_html=True)
with b4:
    if t["ai_categorized"]:
        st.markdown(f'<div style="background:#7c3aed;border-radius:8px;padding:10px 14px;font-size:0.9rem;color:white;font-weight:500">{icon_text("sparkles", "AI Categorized", 15, "white")}</div>', unsafe_allow_html=True)
...
with left_col:
    ...
    if t.get("location"):
        st.markdown(f'<p style="font-size:0.9rem">{icon_text("map-pin", t["location"], 14, "#6b7280")}</p>', unsafe_allow_html=True)
    ...
    ticket_tags = run_query("SELECT tg.name, tg.color FROM ticket_tags tt JOIN tags tg ON tt.tag_id=tg.id WHERE tt.ticket_id=%s", (ticket_id,))
    if ticket_tags:
        tag_html = " ".join([f'<span style="background:{tg["color"]};color:white;padding:3px 12px;border-radius:20px;font-size:0.82rem;font-weight:500">#{tg["name"]}</span>' for tg in ticket_tags])
    ...
    if available:
        ...
        if add_tag != "— select —":
            run_query("INSERT INTO ticket_tags (ticket_id, tag_id) VALUES (%s,%s) ON CONFLICT DO NOTHING", (ticket_id, tag_map[add_tag]), fetch=False)
            st.rerun()

with right_col:
    st.markdown(icon_header("bot", "AI-Suggested Fix", level=3), unsafe_allow_html=True)
    st.markdown(f'<p style="font-size:0.82rem;color:#6b7280;margin-top:-8px">{icon_text("sparkles", "Powered by Google Gemini", 12, "#9ca3af")}</p>', unsafe_allow_html=True)
    ...
    if has_fix:
        ...
        if st.button("Regenerate AI Fix", use_container_width=True):
            with st.spinner("Generating..."):
                ...
                run_query("UPDATE tickets SET ai_suggested_fix=%s WHERE id=%s", (new_fix, ticket_id), fetch=False)
            st.rerun()
    else:
        ...
        if st.button("Generate AI Fix", use_container_width=True, type="primary"):
            with st.spinner("Generating..."):
                new_fix = ai_suggest_fix(t["title"], t["description"], t["category"])
                run_query("UPDATE tickets SET ai_suggested_fix=%s, ai_categorized=TRUE WHERE id=%s", (new_fix, ticket_id), fetch=False)
            st.rerun()
    ...
    with st.form("quick_update"):
        ...
        new_tech = st.selectbox("Assigned To", list(tech_options.keys()),
            index=list(tech_options.keys()).index(t["technician_name"]) if t["technician_name"] in tech_options else 0)
    ...
    resolved_html = f'<div>{icon_text("check-circle", f"Resolved: {resolved_str}", 12, "#16a34a")}</div>' if resolved_str else ""
    st.markdown(
